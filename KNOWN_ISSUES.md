...
- **Accessibility (labeling + contrast)** — every icon-only button and previously-unlabeled input now has an `aria-label`, verified via the browser's actual accessibility tree. Separately, computed real WCAG contrast ratios for the full color palette and fixed 4 failing text colors (used for real content, not decoration) — approved by the user first since this changes visible colors, unlike the labeling work. Real screen-reader/touch-target testing is still a separate, unstarted piece.
