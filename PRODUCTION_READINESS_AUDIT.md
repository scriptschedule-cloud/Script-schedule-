...
- **This is flagged, not yet fixed** — it's a real code change to a function real users may depend on, not a "clearly safe" one-line fix, so it needs to go through the normal phase process with your sign-off on the approach.
...
- **Recommended fix:** add a `corrected_at timestamptz` / `corrected_from text` pair of columns (or a lightweight `dose_event_corrections` history table) so a correction is visible as a correction, not indistinguishable from a first-time entry.
- **Could affect existing features:** Additive schema change; low risk to existing behavior, but touches the medication-logging path.
- **How to test:** log a dose, correct its status, confirm the original value and correction timestamp are both retrievable.
