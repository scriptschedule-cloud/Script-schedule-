...
- **Status: started, not fully closed.** Added a real test suite (`tests/time.test.js`, 15 tests, using Node's built-in `node:test` — no new dependency) covering the timezone-conversion logic from the C3 fix: every timezone/DST/edge case verified earlier by hand is now a permanent, automated regression test (`npm test`, or `node --test`). Extracted the logic itself into `netlify/functions/_shared/time.js` so it's testable in isolation from the HTTP handler. Wired up `.github/workflows/test.yml` — first CI this repo has ever had, runs the suite on every push/PR to `main` (free on GitHub Actions' tier for a suite this size).
  - **Update:** `tests/scheduling.test.js` now covers `courseEndDate`, `isPastCourse`, `isArchived`, and `doseKey` — extracted directly from the real `index.html` source at test time (see `tests/helpers/extractFromIndexHtml.js`) rather than duplicated, so tests can't silently drift from the actual shipped code. 13 tests including real edge cases (durationDays of 0, missing startDate fallback, the inclusive end-date boundary, null vs. unset archivedAt). 28/28 total tests passing.
  - **What's still not covered:** RLS policy tests (verifying user A genuinely cannot read/write user B's household data via a direct request) — needs a real Supabase test project to run against, a bigger lift than adding to this pass.
...
- **Dose history pagination — RESOLVED.** Confirmed nothing in the app ever looks back further than the Report tab's 90-day max range (grepped for any longer-lookback feature — none exists). `loadHouseholdDataFromSupabase()` now fetches only the last 120 days instead of a household's entire history; older rows are untouched in the database. Added migration 0015 for a composite `(household_id, dose_date)` index so the filtered query is actually fast at scale, not just smaller. **Needs the migration run in Supabase** — not yet executed against a database. Verified the new query runs successfully against the live database and a full reload produces no new errors.
...
- **Accessibility — further progress, not fully audited.** Labeling done (see above). Color contrast: computed actual WCAG 2.1 ratios for the full color palette against every background used in the app; found and fixed 4 text colors that failed (2 badly, even for large text) — used for real content (placeholders, section headers, disclosure text), not decoration. Replaced with verified-passing colors in the same hue family, approved by the user first since (unlike the labeling work) this changes visible colors. Touch targets: checked every explicit button/interactive-element size against WCAG 2.2 SC 2.5.8 (24×24px minimum) — found and fixed one real failure (remove-person/pet × button was 16×16px, now 24×24px); the confirm/cancel ✓/× controls (also 24×24px) sit right at the floor, technically compliant. **Still not done** — real screen-reader testing (VoiceOver/TalkBack), dynamic text scaling, and keyboard-navigation flow remain unstarted; none of those can be verified by reading code alone, they need actual assistive-tech testing.
